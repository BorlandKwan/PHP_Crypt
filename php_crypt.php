...
{
    //기본 설정 부여
    const DEFAULT_HASH = 'sha256'; //hash_algos() 에서 확인
    const DEFAULT_METHOD = 'AES-128-ECB'; //openssl_get_cipher_methods() 에서 확인
    ...
    //암호화
    public function encrypt( $plain_text, $password, $method = self::DEFAULT_METHOD, $hash = self::DEFAULT_HASH )
    {
    ...
    //복호화
    public function decrypt( $cipher_text, $password, $method = self::DEFAULT_METHOD, $hash = self::DEFAULT_HASH )
    {
        //입력된 값이 사용 가능한 openssl method 체크
        $methods_check = $this->openssl_methods_check($method);
        if( $methods_check === false ) { return false; }
